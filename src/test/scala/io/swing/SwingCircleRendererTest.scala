...
class SwingCircleRendererTest extends AnyFlatSpec:
  ...
  "Swing Circle" should "be initialized correctly" in:
    val circle = SwingRendererTestUtilities.circleRenderer(radius = 1, color = Color.red, offset = (0, 0))
  ...
  "Swing Circle" should "always have the radius be half the width and height" in :
    val circle = SwingRendererTestUtilities.circleRenderer(radius = 1, color = Color.red)
    for i <- 1 to 10 do

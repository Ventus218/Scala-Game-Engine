...
class SwingOvalRendererTest extends AnyFlatSpec:
  ...
  "Swing Oval" should "be initialized correctly" in :
    val oval = SwingRendererTestUtilities.ovalRenderer(width = 1, height = 2, color = Color.blue, offset = (0, 0))
  ...
  "Swing Oval" should "not be initialized with negative sizes" in :
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.ovalRenderer(width = 0, height = 1, color = Color.red)
    }
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.ovalRenderer(width = -4, height = 5, color = Color.red)
    }
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.ovalRenderer(width = 2, height = -6, color = Color.red)
    }

  "Swing Oval" should "not be initialized with null color" in :
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.ovalRenderer(width = 1, height = 1, color = null)
    }

  "Swing Oval" should "be able to change its properties" in :
    val oval = SwingRendererTestUtilities.ovalRenderer(width = 1, height = 2, color = Color.red, offset = (0, 0))

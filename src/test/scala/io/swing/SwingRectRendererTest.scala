...
class SwingRectRendererTest extends AnyFlatSpec:
  ...
  "Swing Rectangle" should "be initialized correctly" in:
    val rect = SwingRendererTestUtilities.rectRenderer(width = 1, height = 2, color = Color.red, offset = (0, 0))
  ...
  "Swing Rectangle" should "not be initialized with negative sizes" in:
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.rectRenderer(width = 0, height = 0, color = Color.red)
    }
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.rectRenderer(width = -4, height = 5, color = Color.red)
    }
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.rectRenderer(width = 2, height = -6, color = Color.red)
    }

  "Swing Rectangle" should "not be initialized with null color" in:
    an [IllegalArgumentException] shouldBe thrownBy {
      SwingRendererTestUtilities.rectRenderer(width = 1, height = 1, color = null)
    }

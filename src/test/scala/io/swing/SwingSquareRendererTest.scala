...
class SwingSquareRendererTest extends AnyFlatSpec:
  ...
  "Swing Square" should "be initialized correctly" in:
    val square = SwingRendererTestUtilities.squareRenderer(size = 1, color = Color.red, offset = (0, 0))
  ...
  "Swing Square" should "always have the same width and height" in:
    val square = SwingRendererTestUtilities.squareRenderer(size = 1, color = Color.red)
    for i <- 1 to 10 do
      square.shapeWidth = i
